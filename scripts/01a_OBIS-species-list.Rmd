...
# results to dataframe
ncbi_ids3 <- tibble(taxa_query = names(ncbi_add2$accepted_name),
                      ncbi = ncbi_missing$ncbi) %>%
  # remove from lists
  unnest_longer(ncbi, keep_empty = TRUE) %>%
  # remove from next layer of lists
...
ncbi_ids4 <- tibble(taxa_query = names(ncbi_missing$ncbi),
                      ncbi = ncbi_missing$ncbi) %>%
  # remove from lists
  unnest_longer(ncbi, keep_empty = TRUE) %>%
  # remove from next layer of lists
...
ncbi_ids <- ncbi_ids2 %>%
  full_join(ncbi_ids3) %>%
  full_join(ncbi_ids4) %>%
  # add this above to initial mapping
  filter(!(is.na(ncbi_uid))) %>%
  select(ncbi_uid, ncbi_scientificname)
